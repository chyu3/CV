...
class BGR2RGB: #Converts BGR to RGB
    def __call__(self, image):
        return cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

class SquarePad: #Converts into square image by adding black borders
    def __call__(self, image):
...
class Resize(): #Resizes to 128 x 128
...
class Transforms: #Transformation by applying first 3 classes
    def __init__(self):
